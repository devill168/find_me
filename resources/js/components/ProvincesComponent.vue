  <template>
    <div class="container mt-4">
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between mb-3">
            ...
            <button class="btn btn-success" @click="openCreateModal"
              ...
              <i class="fa-solid fa-circle-plus"></i> {{ t('province.createButton') }}
            </button>
          </div>
            ...
            <table class="table table-hover align-middle">
              ...
              <tbody>
                <tr v-for="(p, index) in provinces" :key="p.id || index">
                  ...
                  <td>{{ p.longitude || "-" }}</td>
                  <td class="text-end">
                    <button class="btn btn-sm btn-primary me-2" @click="openEditModal(p)">
                      <i class="fa-solid fa-pen-to-square"></i> {{ t('province.edit') }}
                    </button>
                    <button class="btn btn-sm btn-danger" @click="askDelete(p)">
                      <i class="fa-solid fa-trash"></i> {{ t('province.delete') }}
                    </button>
                  </td>
                </tr>
      ...
      <!-- Create / Edit Modal -->
      <div class="modal fade" id="provinceModal" tabindex="-1" aria-hidden="true" ref="provinceModalEl">
          ...
          <div class="modal-content rounded-4 border-0 shadow modal-gradient">
            <div class="modal-header border-0 flex-column text-center position-relative modal-top">
              ...
              <div v-if="!editMode" class="w-100">
                ...
                <h5 class="modal-title title-border pb-2 mb-4">{{ t('province.createTitle') }}</h5>
              </div>

              <div v-else class="w-100">
                <h5 class="modal-title title-border pb-2 mt-3">{{ t('province.editTitle') }}</h5>
              </div>
            </div>
            ...
            <div class="modal-body pt-0 px-4 px-lg-5 pb-4">
              <form @submit.prevent="editMode ? updateProvince() : createProvince()">
                <div class="row g-4">
                  ...
                  <div class="col-lg-6">
                    <div class="form-panel p-4 rounded-4 shadow-sm">
                      <div class="row g-3">
                        <div class="col-md-12">
                          <label class="form-label custom-label">{{ t('province.provinceNameEnLabel') }}</label>
                          <input type="text" class="form-control custom-input" v-model.trim="form.province_name_en"
                            :placeholder="t('province.provinceNameEnPlaceholder')" required />
                        </div>

                        <div class="col-md-12">
                          <label class="form-label custom-label">{{ t('province.provinceNameKhLabel') }}</label>
                          <input type="text" class="form-control custom-input" v-model.trim="form.province_name_kh"
                            :placeholder="t('province.provinceNameKhPlaceholder')" required />
                        </div>

                        <div class="col-md-12">
                          <label class="form-label custom-label">{{ t('province.provinceCodeLabel') }}</label>
                          <input type="text" class="form-control custom-input bg-secondary-subtle"
                            v-model="form.province_code" readonly
                            :placeholder="t('province.provinceCodePlaceholder')" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label custom-label">{{ t('province.latitudeLabel') }}</label>
                          <input type="number" step="any" class="form-control custom-input" v-model="form.latitude"
                            :placeholder="t('province.latitudePlaceholder')" @input="updateMarkerFromInputs" />
                        </div>

                        <div class="col-md-6">
                          <label class="form-label custom-label">{{ t('province.longitudeLabel') }}</label>
                          <input type="number" step="any" class="form-control custom-input" v-model="form.longitude"
                            :placeholder="t('province.longitudePlaceholder')" @input="updateMarkerFromInputs" />
                        </div>
                  ...
                  <div class="col-lg-6">
                    <div class="map-panel p-4 rounded-4 shadow-sm">
                      <div class="section-title">
                        ...
                        <label class="form-label custom-label mb-1">{{ t('province.mapTitle') }}</label>
                      </div>
                ...
                <div class="modal-footer border-0 px-0 mt-4 justify-content-end gap-2">
                  ...
                  <button v-if="!editMode" type="submit" class="btn btn-success btn-action px-4"
                    :disabled="submitLoading">
                    <i class="fa-solid fa-check me-1"></i>
                    {{ submitLoading ? t('province.saving') : t('province.save') }}
                  </button>

                  <button v-else type="submit" class="btn btn-primary btn-action px-4" :disabled="submitLoading">
                    <i class="fa-solid fa-check me-1"></i>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-body p-4 text-center">
              ...
              <p class="text-muted mb-0">{{ t('province.confirmDeleteMessage') }}</p>
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.showAlert("error", this.t("province.mapError"), this.t("province.mapErrorMessage"))
    }
  },
  ...
  methods: {
    ...
    async fetchNextProvinceCode() {
      try {
        ...
        this.form.province_code = String(res.data.province_code ?? 1)
      } catch (err) {
        console.log("NEXT CODE ERROR:", err.response?.data || err.message)
        this.form.province_code = "1"
      }
    },
    ...
    async createProvince() {
      ...
      try {
        ...
        if (err.response?.status === 422) {
          ...
          const firstMsg = Object.values(errors)?.[0]?.[0] || this.t("province.validationMessage")
        ...
        this.showAlert(
          ...
          err.response?.data?.message || err.message || this.t("province.createFailedMessage")
        )
      } finally {
    ...
    async updateProvince() {
      ...
      try {
        ...
        this.showAlert(
          ...
          err.response?.data?.message || err.message || this.t("province.updateFailedMessage")
        )
      } finally {
        this.submitLoading = false
      }
    ...
    async confirmDelete() {
      ...
      try {
        ...
        this.showAlert(
          ...
          err.response?.data?.message || err.message || this.t("province.deleteFailedMessage")
        )
      } finally {
        this.deleting = false
