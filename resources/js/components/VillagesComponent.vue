        ...
        <div class="card shadow-sm rounded-4">
            <div class="card-body">
                <div class="d-flex justify-content-between mb-3">
                    <h4>{{ t('village.title') }}</h4>
                    <button class="btn btn-success" @click="openCreateModal">
                        <i class="fa-solid fa-circle-plus"></i> {{ t('village.createButton') }}
                    </button>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        <thead class="table-bg">
                            <tr>
                                <th width="80">{{ t('village.tableId') }}</th>
                                ...
                                <th width="180" class="text-end">{{ t('village.action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(v, index) in villages" :key="v.id || index">
                                ...
                                <td>{{ v.od?.od_name_en || "-" }}</td>
                                <td>{{ v.hc?.hc_name_en || "-" }}</td>
                                ...
                                <td class="text-end">
                                    <button class="btn btn-sm btn-primary me-2" @click="openEditModal(v)">
                                        <i class="fa-solid fa-pen-to-square"></i> {{ t('village.edit') }}
                                    </button>
                                    <button class="btn btn-sm btn-danger" @click="askDelete(v)">
                                        <i class="fa-solid fa-trash"></i> {{ t('village.delete') }}
                                    </button>
                                </td>
                            </tr>

                            <tr v-if="villages.length === 0">
                                <td colspan="12" class="text-center text-muted py-4">{{ t('village.noData') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                        ...
                        <form @submit.prevent="editMode ? updateVillage() : createVillage()">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select custom-input" v-model="form.commune_code"
                                            @change="onCommuneChange" required>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label">{{ t('village.selectOD') }}</label>
                                        <select class="form-select custom-input" v-model="form.od_code"
                                            @change="onODChange" required>
                                            <option value="">{{ t('village.selectODOption') }}</option>
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label">{{ t('village.selectHC') }}</label>
                                        <select class="form-select custom-input" v-model="form.hc_code"
                                            @change="onHCChange" required>
                                            <option value="">{{ t('village.selectHCOption') }}</option>
                                            <option v-for="h in filteredHCs" :key="h.id" :value="h.hc_code">
                                                {{ h.hc_name_en }} - {{ h.hc_name_kh }}
                                            </option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control custom-input"
                                            ...
                                            :placeholder="t('village.villageNameEnPlaceholder')" required />
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control custom-input"
                                            ...
                                            :placeholder="t('village.villageNameKhPlaceholder')" required />
                                    </div>

                                    <div class="col-md-6">
                                        <label class="form-label">{{ t('village.villageCodeLabel') }}</label>
                                        <input type="text" class="form-control custom-input bg-body-secondary"
                                            v-model="form.village_code" readonly />
                                    </div>

                                    <div class="col-md-3">
                                        <label class="form-label">{{ t('village.latitudeLabel') }}</label>
                                        <input type="number" step="any" class="form-control custom-input"
                                            v-model="form.latitude" @input="updateMarkerFromInputs" />
                                    </div>

                                    <div class="col-md-3">
                                        <label class="form-label">{{ t('village.longitudeLabel') }}</label>
                                        <input type="number" step="any" class="form-control custom-input"
                                            v-model="form.longitude" @input="updateMarkerFromInputs" />
                                    </div>
                            ...
                            <div class="modal-footer border-0 px-0 mt-4">
                                ...
                                <button v-if="!editMode" type="submit" class="btn btn-success btn-action">
                                    <i class="fa-solid fa-check me-1"></i> {{ t('village.save') }}
                                </button>

                                <button v-else type="submit" class="btn btn-primary btn-action">
                                    <i class="fa-solid fa-check me-1"></i> {{ t('village.update') }}
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    methods: {
        getCurrentLocation() {
            if (!navigator.geolocation) {
                this.showAlert("error", this.t("village.geolocationError"), this.t("village.geolocationNotSupported"))
                return
            }
        ...
        async loadODs() {
            try {
                ...
                console.log(err.response?.data || err.message)
                this.showAlert("error", this.t("village.loadFailed"), this.t("village.loadODFailedMessage"))
            }
        },
        ...
        async loadHealthCenters() {
            try {
                ...
                console.log(err.response?.data || err.message)
                this.showAlert("error", this.t("village.loadFailed"), this.t("village.loadHCFailedMessage"))
            }
        },
        ...
        async onHCChange() {
            ...
            if (!this.form.hc_code) return

            try {
                const res = await axios.get(`/villages/next-code/${this.form.hc_code}`)
        ...
        openEditModal(village) {
            ...
            this.form.od_code = village.od_code || ""

            this.filteredHCs = this.healthCenters.filter(
                (h) => String(h.od_code) === String(this.form.od_code)
            )

            this.form.hc_code = village.hc_code || ""
        ...
        async createVillage() {
            if (
                ...
                this.showAlert("warning", this.t("village.validationError"), this.t("village.fillRequired"))
                return
            }
            ...
            try {
                await axios.post("/villages", {
                    ...
                    od_code: this.form.od_code,
                    hc_code: this.form.hc_code,
        ...
        async confirmDelete() {
            ...
            try {
                ...
                this.villages = this.villages.filter((v) => v.id !== this.deleteTarget.id)

                this.confirmDeleteModal?.hide()
                this.showAlert("success", this.t("village.deletedTitle"), this.t("village.deletedSuccess"))
            } catch (err) {
                console.log(err.response?.data || err.message)
                this.showAlert(
                    "error",
